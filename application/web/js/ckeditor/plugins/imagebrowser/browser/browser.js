...
CkEditorImageBrowser.loadData = function (url, onLoaded) {
	...
	$.getJSON(url, function (list) {
		$.each(list, function (_idx, item) {
			...
			CkEditorImageBrowser.addImage(item);
		});
...
CkEditorImageBrowser.addImage = function (item) {

	CkEditorImageBrowser.images[item.folder].push({
		"imageUrl": item.image,
		"thumbUrl": item.thumb,
...
CkEditorImageBrowser.renderImagesForFolder = function (folderName) {
	...
	$.each(images, function (idx, imageData) {
        ...
        html = html.replace('%imageDimensions%', imageData.dimensions);
